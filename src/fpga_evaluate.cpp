  ...
  void load14Bit(const ap_int<32>* data_dram, ap_int<14>* data_local, int burstLength){
   ...
   void loadStructs(const ap_uint<256>* data_dram, FPGAPsddNodeStruct* data_local, int burstLength){
     ...
     #pragma HLS pipeline
       ...
       data_local[i].children_size = data_dram[i](69, 64);
       data_local[i].children_offset = data_dram[i](115, 96);
    }
   }
   ...
   void load(bool local_variables[MAX_VAR], bool local_instantiation[MAX_VAR], FPGAPsddNodeStruct local_fpga_node_vector[PSDD_SIZE],
     const ap_uint<256> fpga_node_vector[PSDD_SIZE], ap_uint<20> local_prime_vector[TOTAL_CHILDREN], const ap_uint<32> prime_vector[TOTAL_CHILDREN], ap_uint<20> local_sub_vector[TOTAL_CHILDREN], const ap_uint<32> sub_vector[TOTAL_CHILDREN],
...
#pragma HLS pipeline II=3
        uint cur_prime_idx = local_fpga_node_vector[local_prime_vector[local_fpga_node_vector[cur_node_idx].children_offset + i]].node_index_;
        uint cur_sub_idx = local_fpga_node_vector[local_sub_vector[local_fpga_node_vector[cur_node_idx].children_offset + i]].node_index_;
        float tmp = evaluation_cache[local_fpga_node_vector[cur_prime_idx].node_index_] + evaluation_cache[local_fpga_node_vector[cur_sub_idx].node_index_] +  float (local_parameter_vector[local_fpga_node_vector[cur_node_idx].children_offset + i]);
