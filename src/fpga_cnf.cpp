...
namespace {
SddNode *ConstructSddFromCnf(
    const std::vector<std::vector<SddLiteral>> &clauses,
    ...
    for (SddLiteral lit : cur_clause) {
      if (lit < 0) {
        ...
        SddLiteral sdd_variable_index =
            variable_mapping.find(psdd_variable_index)->second;
...
/*
PsddNode *CNF::CompileToSddWithEvidence(const std::unordered_map<uint32_t, bool>
&evid, Vtree *vtree) const { std::vector<std::vector<int>> new_clauses; auto
vtree_variables = VtreeVariables(vtree); std::bitset<MAX_VAR>
vtree_variables_mask; for (auto vtree_variable_index  : vtree_variables){
  ...
  if (existential_quantified_clauses.empty()) {
    ...
    PsddNode *result_node = GetTrueNode(vtree, 1, &node_index, unique_table,
&true_node_map); delete (unique_table); return result_node; } else {
    ...
    SddNode* constraint_with_sdd_variables =
ConstructSddFromCnf(existential_quantified_clauses, variable_mapping, manager);
    PsddNode* converted_psdd = ConvertSddToPsdd(constraint_with_sdd_variables,
vtree, unique_table, & node_index, 1); delete(unique_table);
...
FPGAPsddNode *fpga_CNF::Compile(FPGAPsddManager *psdd_manager, uintmax_t flag_index) const {
  ...
  std::vector<SddLiteral> variables_in_psdd =
      vtree_util::VariablesUnderVtree(psdd_vtree);
  for (SddLiteral psdd_variable_index : variables_in_psdd) {
    psdd_variable_to_sdd_variable[psdd_variable_index] = sdd_index;
    sdd_variable_to_psdd_variable[(uint32_t)sdd_index] =
        (uint32_t)psdd_variable_index;
    sdd_index += 1;
  }
  Vtree *sdd_vtree =
      vtree_util::CopyVtree(psdd_vtree, psdd_variable_to_sdd_variable);
  ...
  SddNode *result = ConstructSddFromCnf(clauses_, psdd_variable_to_sdd_variable,
                                        new_sdd_manager);
  FPGAPsddNode *psdd_result = psdd_manager->ConvertSddToPsdd(
...
/*
bool CNF::CheckConstraintWithPartialInstantiation(const std::bitset<MAX_VAR>
&variable_mask, const std::bitset<MAX_VAR> &variable_instantiation) const { for
(const auto& clause : clauses_){ bool need_check = true; for (int lit : clause){
